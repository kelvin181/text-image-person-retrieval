"""
Simple top-k MLLM reranking on CUHK-PEDES test set.

Gets top-k candidates from IRRA, sends all k images + query to Qwen2-VL in a
single prompt, combines MLLM ranking with IRRA scores, then evaluates metrics.

Usage:
    python icl_rerank/simple_infer.py \\
        --checkpoint logs/.../best_real.pth \\
...
# ---------------------------------------------------------------------------
# Verbatim from infer.py (originally from ICL lines 30–68)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TextPureDataset — from ICL/2025-CVPR-ICL/datasets/bases.py lines 161–176
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Caption embedding — from infer.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Prompt + response parsing — from rerank.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Batched MLLM reranking (multiple images per query)
# ---------------------------------------------------------------------------
def mllm_batch_rerank(llm_obj, sims, img_paths, captions, top_k, alpha, batch_size, logger):
    ...
    for b in tqdm(range(n_batches), desc="MLLM reranking"):
        ...
        for i, response in enumerate(responses):
            q_idx      = start + i
            k_indices  = b_idx[i].numpy()           # gallery indices of top-k
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Simple top-k MLLM reranking on CUHK-PEDES")
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(op.join(args.output_dir, "simple.log")),
        ],
    )
    ...
    if args.load_cache and op.exists(args.load_cache):
        ...
        img_loader  = DataLoader(img_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4)
        gfeats_list = []
        model.eval()
        with torch.no_grad():
    ...
    # ---- save config ----
    config_path = op.join(args.output_dir, 'simple_config.json')
